...
static std::string get_scheme(const gcomm::Protonet& pnet,
                              bool use_ssl, bool dynamic_socket)
{
    if ((use_ssl == true || pnet.tls_service_enabled()) && not dynamic_socket)
    {
...
gcomm::GMCast::GMCast(Protonet& net, const gu::URI& uri,
    ...
    listen_addr_  (
        param<std::string>(
            ...
            get_scheme(pnet_, use_ssl_, dynamic_socket_) + "://0.0.0.0")), // how to make it IPv6 safe?
    initial_addrs_(),
    mcast_addr_   (param<std::string>(conf_, uri, Conf::GMCastMCastAddr, "")),
    bind_ip_      (""),
    mcast_ttl_    (check_range(
                       Conf::GMCastMCastTTL,
                       param<int>(conf_, uri, Conf::GMCastMCastTTL,
                                  Defaults::GMCastMCastTTL),
                       1, 256)),
    listener_     (),
    mcast_        (),
    pending_addrs_(),
    ...
    next_check_   (gu::datetime::Date::monotonic())
{
    ...
    catch (gu::Exception&)
    {
        ...
        listen_addr_ = uri_string(get_scheme(pnet_, use_ssl_, dynamic_socket_), listen_addr_);
    ...
    if ((use_ssl_ == true || pnet_.tls_service_enabled()) && not dynamic_socket_)
    {
...
void gcomm::GMCast::set_initial_addr(const gu::URI& uri)
{
         ...
         i != al.end(); ++i)
    {
        ...
        std::string initial_uri = uri_string(get_scheme(pnet_, use_ssl_, dynamic_socket_), host, port);
        ...
        // resolving sets scheme to tcp, have to rewrite for ssl
        if ((use_ssl_ == true || pnet_.tls_service_enabled()) &&
            not dynamic_socket_)
        {
...
void gcomm::GMCast::add_or_del_addr(const std::string& val)
{
    if (val.compare(0, 4, "add:") == 0)
    {
        ...
        std::string addr(gu::net::resolve(uri_string(get_scheme(pnet_, use_ssl_, dynamic_socket_),
                                                     uri.get_host(),
